...
const linkClass = ({ isActive }: { isActive: boolean }) =>
	`block px-4 py-2 text-sm rounded ${isActive ? 'bg-gray-700 font-semibold' : 'hover:bg-gray-700'}`;

const deviceLinkClass = ({ isActive }: { isActive: boolean }) =>
	`block px-4 py-2 font-mono text-sm rounded ${isActive ? 'bg-gray-700 font-semibold' : 'hover:bg-gray-700'}`;
				...
				<ul>
					...
					{visible.map((d, i) => (
						<li key={d.device_id} ref={i === 0 ? measureRow : undefined}>
							<NavLink to={`/device/${d.device_id}`} className={deviceLinkClass}>
								{d.device_id}
							</NavLink>
						</li>
					))}
